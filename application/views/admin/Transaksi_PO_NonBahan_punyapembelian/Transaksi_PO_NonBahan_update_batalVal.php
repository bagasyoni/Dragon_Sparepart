...
<div class="container-fluid">
	...
	<form id="pononbahan" name="pononbahan" action="<?php echo base_url('admin/Transaksi_PO_NonBahan/update_aksi'); ?>" class="form-horizontal needs-validation" method="post" novalidate>
		...
		<div class="row">
			...
			<div class="col-md-1">
				<input <?php if ($rowh->TTD1 == !0) echo 'class="form-control TGL text_input" readonly'; ?> type="text" class="form-control text_input" id="TGL" name="TGL" data-date-format="dd-mm-yyyy" value="<?php echo date('d-m-Y', strtotime($rowh->TGL, TRUE)); ?>" <?= ($this->session->userdata['ha_e_ttd'] != '1') ? "readonly" : "" ?>>
			</div>
			<div class="col-md-1">
		...
		<div class="col-md-12" style="justify-content: center;">
			<div class="form-group row" style="justify-content: center;">
				<div class="col-md-2">
					...
					<input <?php if ($rowh->TTD1 == !0) echo 'hidden'; ?> class="form-control text_input PIN" id="PIN" name="PIN" type="password" maxlength="6" value="" placeholder="PIN ...">
				</div>
						...
						<tbody>
							...
							?>
								<tr>
									...
									<td><input <?php if ($rowh->TTD1 == !0) echo 'readonly'; ?> name="HARGA[]" onclick="select()" onchange="hitung()" id="HARGA<?php echo $no; ?>" value="<?php echo number_format($row->HARGA, 2, '.', ','); ?>" type="text" class="form-control HARGA rightJustified text-primary"></td>
		...
		<div class="row">
			<div class="col-md-9">
				<div class="btn-group">
					...
					<button type="button" onclick="btbtlVerifikasi()" class="btn btn-secondary">Batal Validasi</button>
				</div>
			</div>
			<div class="col-md-1">
...
<script>
	...
	function btVerifikasi() {
		if ($('#PIN').val() == '<?= $this->session->userdata['pin'] ?>') {
			if (confirm("Yakin Posting? \n Posting hanya bisa Posting Data, tidak untuk edit data.")) {
				...
				window.location.replace("<?php echo base_url('admin/Transaksi_PO_NonBahan/verifikasi_ttd1/' . $rowh->ID) ?>");
			}
		} else {
	...
	function btbtlVerifikasi() {
		if (confirm("Yakin Batal Validasi?")) {

			// document.getElementById("transaksipemesanan").submit();
			window.location.replace("<?php echo base_url('admin/Transaksi_PO_NonBahan/btlval/' . $rowh->ID) ?>");
		}
